...
- **MCP config:** single source `agent/workspace-root/.cursor/mcp.json` → sync to parent **and** `agent/.cursor/mcp.json` (paths differ by open mode). See `agent/agent_docs/mcp-workspace-layout.md`. Verify: `.\tools\scripts\test-mcp-config.ps1`
...
- **Author:** `rootsandfruit/set-post-author` (after plugin deploy); cache purge still manual
