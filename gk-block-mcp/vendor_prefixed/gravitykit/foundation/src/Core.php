...
namespace GravityKit\BlockMCP\Foundation;

use GravityKit\BlockMCP\Foundation\Components\NewsletterSignup;
use GravityKit\BlockMCP\Foundation\Components\SecureDownload;
use GravityKit\BlockMCP\Foundation\Integrations\GravityForms;
use GravityKit\BlockMCP\Foundation\Integrations\HelpScout;
use GravityKit\BlockMCP\Foundation\Integrations\TrustedLogin;
use GravityKit\BlockMCP\Foundation\Scheduler\Overview\JobOverview;
...
use GravityKit\BlockMCP\Foundation\CLI\CLI;
use GravityKit\BlockMCP\Foundation\WP\PluginActivationHandler;
use GravityKit\BlockMCP\Foundation\Settings\Framework as SettingsFramework;
use GravityKit\BlockMCP\Foundation\Licenses\Framework as LicensesFramework;
...
use GravityKit\BlockMCP\Foundation\WP\RESTController;
use GravityKit\BlockMCP\Foundation\Notices\NoticeManager as Notices;
use GravityKit\BlockMCP\Foundation\Settings\WPDebugSettings;
...
class Core
{
    ...
    private function __construct($plugin_file, $arguments = [])
    {
        ...
        $this->_registered_plugins = [$plugin_file => array_merge(['plugin_file' => $plugin_file, 'text_domain' => CoreHelpers::get_plugin_data($plugin_file)['TextDomain'], 'foundation_version' => self::VERSION, 'loads_foundation' => true, 'no_admin_menu' => false], $arguments)];
        add_filter('gk/foundation/get-instance', function ($passed_instance) use ($plugin_file) {
        ...
        add_action('plugins_loaded', static function () {
            ...
            $message = strtr(_n('[plugins] did not load correctly. Please deactivate and reactivate it to resolve this issue.', '[plugins] did not load correctly. Please deactivate and reactivate them to resolve this issue.', $count, 'gk-foundation'), ['[plugins]' => '<strong>' . esc_html($list) . '</strong>']);
            $support_link = '<a href="https://www.gravitykit.com/support/" target="_blank" rel="noopener noreferrer">' . esc_html__('contact support', 'gk-foundation') . '</a>';
            // translators: [link] is replaced with a support link.
            $message .= ' ' . strtr(esc_html__('If the problem persists, [link].', 'gk-foundation'), ['[link]' => $support_link]);
            Notices::get_instance()->add_runtime(['namespace' => 'gk-foundation', 'slug' => 'foundation-load-failure', 'message' => $message, 'severity' => 'error', 'dismissible' => false, 'capabilities' => ['manage_options']]);
        }, PHP_INT_MAX);
    }
    /**
    ...
    public static function register($plugin_file, $arguments = [])
    {
        ...
        if (wp_doing_ajax() && LicensesFramework::AJAX_ROUTER === ($_REQUEST['ajaxRouter'] ?? '') && CoreHelpers::version_compare($_REQUEST['frontendFoundationVersion'] ?? 0, self::VERSION, '<')) {
            return;
        }
        ...
        if (is_null(self::$_instance)) {
            ...
            self::$_instance->_registered_plugins[$plugin_file] = array_merge(['plugin_file' => $plugin_file, 'text_domain' => $text_domain, 'foundation_version' => self::$_instance->get_plugin_foundation_version($plugin_file), 'has_foundation' => false, 'loads_foundation' => false, 'no_admin_menu' => false], $arguments);
        }
        add_filter('doing_it_wrong_trigger_error', function ($trigger_error, $function_name, $message) use ($text_domain) {
    ...
    public function set_registered_plugins($plugins)
    {
        ...
        foreach ($plugins as $plugin) {
            if (is_array($plugin)) {
                ...
                $registered_plugins[$plugin] = ['plugin_file' => $plugin, 'text_domain' => CoreHelpers::get_plugin_data($plugin)['TextDomain'], 'foundation_version' => $this->get_plugin_foundation_version($plugin), 'loads_foundation' => false];
            }
        }
    ...
    public function init()
    {
        ...
        $this->_components = ['translations' => TranslationsFramework::get_instance(), 'newsletter' => NewsletterSignup::get_instance(), 'settings' => SettingsFramework::get_instance(), 'licenses' => LicensesFramework::get_instance(), 'logger' => LoggerFramework::get_instance(), 'admin_menu' => AdminMenu::get_instance(), 'ajax_router' => AjaxRouter::get_instance(), 'rest_controller' => RESTController::get_instance(), 'notices' => Notices::get_instance(), 'encryption' => Encryption::get_instance(), 'trustedlogin' => TrustedLogin::get_instance(), 'helpscout' => HelpScout::get_instance(), 'gravityforms' => GravityForms::get_instance(), 'secure_download' => SecureDownload::get_instance(), 'scheduler' => JobScheduler::get_instance(), 'job_overview' => JobOverview::get_instance()];
        ...
        \GravityKit\BlockMCP\Foundation\Licenses\Integrity\PackageVerifier::init();
    ...
    {
        new WPDebugSettings();
        add_filter('gk/foundation/settings/data/plugins', function ($plugins, $payload = []) {
            ...
            $default_settings = ['group_gk_products' => 0, 'top_level_menu_action' => $this->licenses()::ID, 'top_level_menu_position' => class_exists('GFAPI') ? 'gf_edit_forms' : 'upload.php', 'support_email' => get_bloginfo('admin_email'), 'support_port' => 1, 'no_conflict_mode' => 1, 'background_processing' => 1, 'show_background_jobs' => 0, 'scheduler_loopback_url' => '', 'powered_by' => 0, 'beta' => 0];
            ...
            $top_level_menu_action_choices = array_map(function ($menu_item) {
                ...
                return ['title' => $menu_item['menu_title'], 'value' => $menu_item['id']];
            }, $admin_menu_items);
            ...
            if (defined('GV_PLUGIN_VERSION') && CoreHelpers::version_compare(GV_PLUGIN_VERSION, '2.16', '<')) {
                ...
                $notice_2 = strtr(esc_html_x('As such, the settings below will not apply to GravityView pages and you will have to continue using the [link]old settings[/link] until an updated version of the plugin is available. We apologize for the inconvenience as we work to update our products in a timely fashion.', 'Placeholders inside [] are not to be translated.', 'gk-foundation'), ['[link]' => '<a href="' . admin_url('edit.php?post_type=gravityview&page=gravityview_settings') . '" class="text-blue-gv underline hover:text-gray-900 focus:text-gray-900 focus:no-underline focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900">', '[/link]' => '</a>']);
                $html = <<<HTML
                ...
                HTML;
                $general_settings[] = ['id' => 'legacy_settings_notice', 'html' => $html];
            }
            $gravitykit_menu_position_choices = [];
            foreach (AdminMenu::get_menus() as $menu) {
                $gravitykit_menu_position_choices[] = ['title' => $menu['title'], 'value' => $menu['id']];
            }
            $general_settings = array_merge($general_settings, [['id' => 'group_gk_products', 'type' => 'checkbox', 'value' => Arr::get($gk_settings, 'group_gk_products', $default_settings['group_gk_products']), 'choices' => $top_level_menu_action_choices, 'title' => esc_html__('Group GravityKit Products', 'gk-foundation'), 'description' => esc_html__('Aggregate all GravityKit products into a single entry on the Plugins page for a cleaner view and easier management.', 'gk-foundation')], ['id' => 'top_level_menu_position', 'type' => 'select', 'title' => esc_html__('GravityKit Menu Position', 'gk-foundation'), 'description' => esc_html__('Select the menu below which to place the GravityKit menu.', 'gk-foundation'), 'value' => Arr::get($gk_settings, 'top_level_menu_position', $default_settings['top_level_menu_position']), 'choices' => $gravitykit_menu_position_choices], ['id' => 'top_level_menu_action', 'type' => 'select', 'value' => $top_level_menu_action_value, 'choices' => $top_level_menu_action_choices, 'title' => esc_html__('GravityKit Menu Page', 'gk-foundation'), 'description' => esc_html__('Select the page to open when clicking the GravityKit menu.', 'gk-foundation')], ['id' => 'powered_by', 'type' => 'checkbox', 'value' => Arr::get($gk_settings, 'powered_by', $default_settings['powered_by']), 'title' => esc_html__('Display "Powered By" Link', 'gk-foundation'), 'description' => esc_html__('A "Powered by GravityKit" link will be displayed below some GravityKit products. Help us spread the word!', 'gk-foundation')], ['id' => 'affiliate_id', 'type' => 'number', 'value' => Arr::get($gk_settings, 'affiliate_id'), 'title' => esc_html__('Affiliate ID', 'gk-foundation'), 'description' => strtr(esc_html_x('Earn money when people clicking your links become GravityKit customers. [link]Register as an affiliate[/link]!', 'Placeholders inside [] are not to be translated.', 'gk-foundation'), ['[link]' => '<a href="https://www.gravitykit.com/account/affiliates/?utm_source=in-plugin&utm_medium=setting&utm_content=Register%20as%20an%20affiliate" class="underline" rel="external">', '[/link]' => '</a>']), 'requires' => ['id' => 'powered_by', 'operator' => '=', 'value' => '1']], ['id' => 'beta', 'type' => 'checkbox', 'value' => Arr::get($gk_settings, 'beta', $default_settings['beta']), 'title' => esc_html__('Become a Beta Tester', 'gk-foundation'), 'description' => esc_html__('You will have early access to the latest GravityKit products. There may be bugs! If you encounter an issue, report it to help make GravityKit products better!', 'gk-foundation')]]);
            $support_settings = [['id' => 'support_email', 'type' => 'text', 'required' => true, 'value' => Arr::get($gk_settings, 'support_email', $default_settings['support_email']), 'title' => esc_html__('Support Email', 'gk-foundation'), 'description' => esc_html__('In order to provide responses to your support requests, please provide your email address.', 'gk-foundation'), 'validation' => [['rule' => 'required', 'message' => esc_html__('Support email is required', 'gk-foundation')], ['rule' => 'email', 'message' => esc_html__('Please provide a valid email address', 'gk-foundation')]]], ['id' => 'support_port', 'type' => 'checkbox', 'value' => Arr::get($gk_settings, 'support_port', $default_settings['support_port']), 'title' => esc_html__('Show Support Port', 'gk-foundation'), 'description' => esc_html__('The Support Port provides quick access to how-to articles and tutorials. For administrators, it also makes it easy to contact support.', 'gk-foundation') . strtr(esc_html_x('[image]Support Port icon[/image]', 'Placeholders inside [] are not to be translated.', 'gk-foundation'), ['[image]' => '<div style="margin-top: 1em; width: 7em;">![', '[/image]' => '](' . CoreHelpers::get_assets_url('support-port-icon.jpg') . ')</div>']), 'markdown' => true]];
            $technical_settings = [['id' => 'no_conflict_mode', 'type' => 'checkbox', 'value' => Arr::get($gk_settings, 'no_conflict_mode', $default_settings['no_conflict_mode']), 'title' => esc_html__('Enable No-Conflict Mode', 'gk-foundation'), 'description' => esc_html__('No-conflict mode prevents extraneous scripts and styles from being printed on GravityKit admin pages, reducing conflicts with other plugins and themes.', 'gk-foundation')], ['id' => 'background_processing', 'type' => 'checkbox', 'value' => Arr::get($gk_settings, 'background_processing', $default_settings['background_processing']), 'title' => esc_html__('Enable Background Processing', 'gk-foundation'), 'description' => strtr(esc_html_x('Allow GravityKit products to [url]process jobs in the background[/url]. Disable to stop background jobs from running.', 'Placeholders inside [] are not to be translated.', 'gk-foundation'), ['[url]' => '<a class="underline" href="https://docs.gravitykit.com/article/2150-background-processing" rel="noopener noreferrer" target="_blank">', '[/url]' => '<span class="screen-reader-text"> ' . esc_html__('(This link opens in a new window.)', 'gk-foundation') . '</span></a>'])], ['id' => 'show_background_jobs', 'type' => 'checkbox', 'value' => Arr::get($gk_settings, 'show_background_jobs', $default_settings['show_background_jobs']), 'title' => esc_html__('Show Background Jobs', 'gk-foundation'), 'description' => esc_html__('Show the Background Jobs page in the GravityKit menu, where you can view jobs and their execution status.', 'gk-foundation'), 'requires' => ['id' => 'background_processing', 'operator' => '=', 'value' => '1']], ['id' => 'scheduler_loopback_url', 'type' => 'text', 'value' => Arr::get($gk_settings, 'scheduler_loopback_url', $default_settings['scheduler_loopback_url']), 'title' => esc_html__('Loopback URL Override', 'gk-foundation'), 'description' => esc_html__('Override the base URL used for internal HTTP requests. Leave empty to use the site URL. Only change this if background jobs fail because the server cannot reach itself.', 'gk-foundation'), 'placeholder' => 'https://example.com', 'requires' => ['id' => 'background_processing', 'operator' => '=', 'value' => '1']]];
            $all_settings = [self::ID => ['id' => self::ID, 'title' => 'GravityKit', 'defaults' => $default_settings, 'icon' => CoreHelpers::get_assets_url('gravitykit-icon.png'), 'sections' => [['title' => esc_html__('General', 'gk-foundation'), 'settings' => $general_settings], ['title' => esc_html__('Support', 'gk-foundation'), 'settings' => $support_settings], ['title' => esc_html__('Technical', 'gk-foundation'), 'settings' => $technical_settings]]]];
            /**
    ...
    public function inline_scripts_and_styles()
    {
        ...
        if (!empty($inline_scripts)) {
            ...
            wp_register_script(self::ID, false, $dependencies);
        ...
        if (!empty($inline_styles)) {
            ...
            wp_register_style(self::ID, false);
    ...
    public function __call($name, array $arguments = [])
    {
        ...
        if ('helpers' === $name) {
            return (object) ['core' => new CoreHelpers(), 'array' => new Arr()];
        }
    ...
    public function get_foundation_information(): array
    {
        ...
        $loaded_by_message = strtr(_x('GravityKit Foundation [version] (loaded by [plugin]).', 'Placeholders inside [] are not to be translated.', 'gk-foundation'), ['[version]' => $version, '[plugin]' => $source_plugin_name]);
        /**
        ...
        return ['version' => $version, 'source_plugin' => $source_plugin, 'registered_plugins' => $this->_registered_plugins, 'loaded_by_message' => $loaded_by_message, 'show_loaded_by_message' => $show_loaded_by_message];
    }
    /**
    ...
    public function get_latest_foundation_version_from_registered_plugins($text_domain_to_exclude = null)
    {
        ...
        $foundation_versions = array_map(function ($plugin) {
    ...
    private function detect_namespace_conflict()
    {
        ...
        $this->notices()->add_runtime(['namespace' => 'gk-foundation', 'slug' => 'namespace-conflicts', 'message' => strtr(
            // translators: [plugins] is replaced with a list of plugin names.
            _n('[plugins] contains both namespaced and non-namespaced Foundation, which may cause conflicts with the standalone Foundation plugin.', '[plugins] contain both namespaced and non-namespaced Foundation, which may cause conflicts with the standalone Foundation plugin.', count($conflicting_plugins), 'gk-foundation'),
            ['[plugins]' => '<strong>' . implode(', ', $conflicting_plugins) . '</strong>']
        ), 'severity' => 'warning', 'context' => 'all', 'dismissible' => false, 'capabilities' => ['manage_options']]);
    }
}