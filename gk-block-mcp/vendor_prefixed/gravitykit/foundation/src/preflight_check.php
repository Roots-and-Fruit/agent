...
function is_disabled_via_url($plugin_file)
{
    ...
    $plugin_data = \GravityKit\BlockMCP\Foundation\Helpers\Core::get_plugin_data($plugin_file);
    ...
    $disable_loading = $_GET['gk_disable_loading'] ?? null;
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended
...
{
    $plugin_data = \GravityKit\BlockMCP\Foundation\Helpers\Core::get_plugin_data($plugin_file);
    $meets_requirement = (bool) \GravityKit\BlockMCP\Foundation\Helpers\Core::version_compare(phpversion(), $min_php_version, '>=');
    if (!$show_notice) {
        return $meets_requirement;
    }
    if (!$meets_requirement) {
        $notice = strtr(esc_html_x('[plugin] requires PHP [version] or newer.', 'Placeholders inside [] are not to be translated.', 'gk-foundation'), ['[plugin]' => $plugin_data['Name'], '[version]' => $min_php_version]);
